...
export default function RosterPage() {
  ...
  const SkaterTable = ({ data, isArchived }: { data: Skater[], isArchived?: boolean }) => (
    <div className={`overflow-hidden bg-card shadow rounded-lg ${isArchived ? 'opacity-60' : ''}`}>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-border">
          <thead className="bg-secondary">
            <tr>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Name</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Federation/Country</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Age</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Discipline</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Level</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Status</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-card divide-y divide-border">
            ...
            {data.map((skater) => (
              <tr key={skater.id} className="cursor-pointer hover:bg-secondary" onClick={() => router.push(`/dashboard/skaters/${skater.id}`)}>
                <td className="px-4 py-4 text-sm font-medium text-foreground whitespace-nowrap">{skater.full_name}</td>
                ...
                <td className="px-4 py-4 text-sm text-muted-foreground whitespace-nowrap">{skater.discipline}</td>
                <td className="px-4 py-4 text-sm text-muted-foreground whitespace-nowrap">{skater.current_level}</td>
    ...
    <div className={`overflow-hidden bg-card shadow rounded-lg ${isArchived ? 'opacity-60' : ''}`}>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-border">
          <thead className="bg-secondary">
            <tr>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Team</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Federation/Country</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Discipline</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Level</th>
              <th className="px-4 py-3 text-xs font-medium tracking-wider text-left text-muted-foreground uppercase">Status</th>
          ...
          <tbody className="bg-card divide-y divide-border">
            ...
            {data.map((team) => (
              <tr key={team.id} className="hover:bg-secondary">
                ...
                <td className="px-4 py-4 text-sm text-muted-foreground whitespace-nowrap">{team.current_level}</td>
                <td className="px-4 py-4 text-sm whitespace-nowrap">
